...
func (p *Protocol) updateChain(nft *nftables.NFTables, tab *nftables.Table, chain *nftables.Chain, attrs map[uint16]nlmsg.BytesView, family stack.AddressFamily, flags uint16, policy uint8, updateFlags uint32) *syserr.AnnotatedError {
	...
	if hookDataBytes, ok := attrs[linux.NFTA_CHAIN_HOOK]; ok {
		if !isBaseChain {
			return syserr.NewAnnotatedError(syserr.ErrBusy, "hook attribute specified for regular chain")
		}

		updateBCInfo, err := p.chainParseHook(chain, family, nlmsg.AttrsView(hookDataBytes), attrs)
		if err != nil {
			return err
		}
	...
	if _, ok := attrs[linux.NFTA_CHAIN_COUNTERS]; ok {
		...
		log.Warningf("Nftables: chain counters attribute is unimplemented")
	}
...
func getChain(tab *nftables.Table, attrs map[uint16]nlmsg.BytesView) (*nftables.Chain, *syserr.AnnotatedError) {
	...
	if chainHandleBytes, ok := attrs[linux.NFTA_CHAIN_HANDLE]; ok {
	...
	} else if chainIDBytes, ok := attrs[linux.NFTA_CHAIN_ID]; ok {
		chainID, ok := chainIDBytes.Uint32()
		if !ok {
			return nil, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Chain ID attribute is malformed or not found")
		}
		...
		return nil, syserr.NewAnnotatedError(syserr.ErrInvalidArgument, "Nftables: Attributes for chain must contain a chain name, handle, or chain id")
	}
	return chain, nil
}
...
func (p *Protocol) addChain(nft *nftables.NFTables, attrs map[uint16]nlmsg.BytesView, tab *nftables.Table, family stack.AddressFamily, policy uint8, chainFlags uint32) (*nftables.Chain, *syserr.AnnotatedError) {
	...
	if nameBytes, ok := attrs[linux.NFTA_CHAIN_NAME]; ok {
		if chainFlags&linux.NFT_CHAIN_BINDING != 0 {
			return nil, syserr.NewAnnotatedError(syserr.ErrNotSupported, "Nftables: Chain binding attribute is not supported for named chains")
		}
