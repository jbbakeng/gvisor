...
func TestExecJobControl(t *testing.T) {
	...
	// Wait until a cat is running and there's a second sleep besides the container's own, that is,
	// until the sleep 100 | cat pipeline is actually up.
	if err := testutil.Poll(func() error {
		out, err := d.Exec(ctx, dockerutil.ExecOpts{}, "sh", "-c",
			"pgrep -x cat >/dev/null && [ \"$(pgrep -x sleep | wc -l)\" -ge 2 ]")
		if err != nil {
