...
TEST_F(PtyTest, PartialBadBuffer) {
  ...
  // Why NOLINT: The 'addr' pointer is safely printed in the diagnostic string
  // because it evaluates the address value and never dereferences it.
  // Note: Dereferencing 'addr' after a successful munmap would cause a
  // segmentation fault.
  EXPECT_THAT(munmap(addr, 2 * kPageSize), SyscallSucceeds())
      << addr;  // NOLINT(bugprone-use-after-move)
}
